...
| 3D & Canvas      | Three.js + R3F + React Flow     | Interactive 3D graphics, WebGPU acceleration, real-time graph  |
| Auth             | Clerk                           | User identity and route protection                             |
| Database         | Prisma + PostgreSQL             | Relational metadata: projects, collaborators, specs, task runs |
| Multiplayer      | Liveblocks                      | Real-time presence, live cursors, and schema sync              |
...
- `lib` — Shared infrastructure: Prisma client, 3D memory disposal helpers, WebGPU check, and utilities.
- `components/canvas` — React Three Fiber (R3F) 3D mesh rendering layer only.
- `components/ui` — 2D DOM UI overlays layered over 3D canvas via CSS positioning.
- `public/models` — Optimized `.glb` 3D model assets for web streaming.
